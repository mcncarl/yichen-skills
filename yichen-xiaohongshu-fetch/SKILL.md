...
- 仓库内：`python3 yichen-volc-asr/scripts/transcribe.py '<视频文件路径>'`
- 已安装 skill：`python3 <skills-dir>/yichen-volc-asr/scripts/transcribe.py '<视频文件路径>'`
...
- 没有字幕 → 下载视频后调用 `yichen-volc-asr` 转写
- yichen-volc-asr 调用失败 → 记录错误，提示用户可手动转写后用"正文口播稿"字段
...
**视频笔记：抓页面 → curl下载视频直链 → yichen-volc-asr转写 → LLM分段 → 写视频对标库 → 按用户许可清理临时文件**
