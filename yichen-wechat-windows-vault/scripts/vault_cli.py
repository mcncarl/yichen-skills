...
def resolve_db_dir() -> Path | None:
    ...
    key_file = DEFAULT_VAULT_DIR / "keys.json"
    if key_file.exists():
        value = load_json(key_file).get("_db_dir")
        if value:
            return Path(value)
    return None
...
def command_moments(args: argparse.Namespace) -> None:
    ...
    with connect(sns_db) as con:
        if usernames:
            ...
            rows = con.execute(
                f"SELECT tid, user_name, content FROM SnsTimeLine WHERE user_name IN ({placeholders})",
                tuple(usernames),
            )
        else:
            rows = con.execute("SELECT tid, user_name, content FROM SnsTimeLine")
        for row in rows:
            if not row["content"]:
                continue
            try:
