...
def copy_html_to_clipboard_windows(html: str) -> bool:
    """
    Copy HTML to Windows clipboard using win32clipboard with CF_HTML format.
    Explicitly specifies UTF-8 encoding to prevent GBK mojibake issues.
    """
    try:
        import win32clipboard
        import win32con

        # 1. Generate CF_HTML format (Windows clipboard standard)
        # CF_HTML requires specific header format with byte offsets
        html_utf8 = html.encode('utf-8', errors='replace')

        # CF_HTML header template
        # StartHTML/EndHTML: start and end positions of entire content
        # StartFragment/EndFragment: start and end positions of actual HTML fragment
        cf_html_template = (
            "Version:0.9\r\n"
        ...
        html_decoded = html_utf8.decode('utf-8')
        header_template = cf_html_template.split('{html_content}')[0]
        footer = cf_html_template.split('{html_content}')[1]

        # Temporary fill to calculate length
        temp_header = header_template.format(
            start_html=0,
            end_html=9999999999,
            start_fragment=0,
            end_fragment=9999999999
        )

        start_html = 0
        start_fragment = len(temp_header.encode('utf-8'))
        end_fragment = start_fragment + len(html_utf8)
        end_html = end_fragment + len(footer.encode('utf-8'))

        # Generate final CF_HTML content
        cf_html_final = cf_html_template.format(
            start_html=start_html,
            end_html=end_html,
            start_fragment=start_fragment,
            end_fragment=end_fragment,
            html_content=html_decoded
        )

        cf_html_bytes = cf_html_final.encode('utf-8', errors='replace')
        ...
        try:
            ...
            # Set CF_HTML data (UTF-8 byte stream)
            win32clipboard.SetClipboardData(CF_HTML, cf_html_bytes)

            # Also set plain text as fallback (UTF-16 LE)
            # Windows clipboard CF_UNICODETEXT uses UTF-16 LE
            plain_text = html  # Keep HTML tags as plain text fallback
            win32clipboard.SetClipboardData(win32con.CF_UNICODETEXT, plain_text)

            print("✅ HTML copied to clipboard using CF_HTML format (UTF-8)", file=sys.stderr)
            return True
        ...
        print("❌ win32clipboard not available, falling back to clip-util", file=sys.stderr)
        # Fallback to original clip-util method (preserve backward compatibility)
        try:
            from clipboard import Clipboard
            with Clipboard() as clipboard:
                clipboard["html"] = html
            print("⚠️ Used clip-util fallback (may have encoding issues)", file=sys.stderr)
            return True
        except Exception as e2:
            print(f"❌ All clipboard methods failed: {e2}", file=sys.stderr)
            return False

    except Exception as e:
        print(f"❌ Error copying HTML to clipboard: {e}", file=sys.stderr)
        return False
