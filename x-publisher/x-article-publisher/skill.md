...
```bash
cd "文章所在目录"
export MARKDOWN_FILE="example 2026"  # 空格分隔的多个关键词
...
**模式2：目录路径（适合单文件目录）**
```bash
export MARKDOWN_FILE="<OBSIDIAN_VAULT>/posts/topic-folder"
python ~/.claude/skills/x-publisher/scripts/parse_markdown.py
```
- ✅ 如果目录只有一个 .md 文件，自动使用
- ⚠️ 多个文件时会列出所有可用文件并报错
...
**模式3：完整文件路径（最精确）**
```bash
export MARKDOWN_FILE="<OBSIDIAN_VAULT>/posts/example-article-2026.md"
python ~/.claude/skills/x-publisher/scripts/parse_markdown.py
```
- ✅ 直接指定文件
- ✅ 支持中文、空格、特殊字符
...
1.1 设置文件路径环境变量（三选一）
    ...
    方式A - 关键词匹配（推荐）：
    cd "<OBSIDIAN_VAULT>/posts/topic-folder"
    export MARKDOWN_FILE="example 2026"

    方式B - 目录路径：
    export MARKDOWN_FILE="<OBSIDIAN_VAULT>/posts/topic-folder"

    方式C - 完整路径：
    export MARKDOWN_FILE="<OBSIDIAN_VAULT>/posts/topic-folder/example-article-2026.md"
...
2. **内容图片插入** (使用剪贴板 + Ctrl+V)
   ...
   - ✅ **直接使用原始路径**：`python copy_to_clipboard.py image "<IMAGE_PATH>/example.jpg"`
   - ❌ **错误做法**：不要预先复制所有图片到 D 盘，完全没必要！

3. **最佳实践**
   ```python
   # ✅ 正确：直接使用原始路径
   python copy_to_clipboard.py image "<OBSIDIAN_VAULT>/articles/images/example.jpg" --quality 85

   # ❌ 错误：没必要复制
   cp "<OBSIDIAN_VAULT>/articles/images/example.jpg" "D:/temp.jpg"
